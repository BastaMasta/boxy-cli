...
pub struct Boxy<'a> {
    ...
    tot_seg: usize,
    terminal_width_offset: Option<usize>,
}
...
impl<'a> Boxy<'a> {
    ...
    pub fn display(&mut self) {
        ...
        let disp_width = if self.fixed_width != 0 {
            self.fixed_width - 2 - *self.terminal_width_offset.get_or_insert(0)
        } else {
            term_size - self.ext_padding.lr() - 2 - *self.terminal_width_offset.get_or_insert(0)
        };
...
impl<'a> BoxyBuilder<'a> {
    ...
    ///
    pub fn set_terminal_width_offset(mut self, offset: usize) -> Self {
        self.terminal_width_offset = Some(offset);
        self
    }
